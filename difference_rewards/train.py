...
def train(num_iterations: int = 3000, num_envs: int = 32, device: str = 'cuda', resume_path: str = ""):
    ...
    # RND curiosity, gated to the losing team. Dual channel enabled from the
    # start (interaction channel active) to give a richer state-novelty signal
    # alongside the targeted grab/lock entropy that drives box-interaction
    # exploration (phase-2 discovery). Kept at a modest weight so the novelty
    # bonus can't pull hiders out of the room (which would break phase-1).
    ...
    try:
        for update in tqdm(range(start_update, num_iterations), desc="Updates"):
            ...
            for step in range(EPISODE_LEN):
                ...
                next_obs, next_gstate, rext, rdr, dones = vec_env.step(actions.cpu().numpy())
