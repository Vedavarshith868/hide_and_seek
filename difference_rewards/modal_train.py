...
# Each experiment gets its OWN volume so nothing is ever overwritten:
#   hide-and-seek-data : earlier GAE/COMA runs (preserved)
#   hide-and-seek-dr   : first DR run (entropy-confounded; preserved)
#   hide-and-seek-dr2  : clean DR run (corrected entropy, kappa=1.0)
vol = modal.Volume.from_name("hide-and-seek-dr2", create_if_missing=True)
...
def _latest_valid_checkpoint(ckpt_dir: str = "/data/checkpoints") -> str:
    ...
    import re
    import torch
    from constants import LIDAR_DIM   # current arch's lidar-embedder input dim
    ...
    for _, f in cands:
        try:
            ...
            # Only resume a checkpoint whose obs dimensionality matches the
            # CURRENT architecture — otherwise a crash-restart would try to
            # load an incompatible old run (e.g. 32-dim lidar vs current 74).
            if w.shape[1] == LIDAR_DIM and "optimizer" in ck:
                return f
        except Exception:
            continue
    return ""
