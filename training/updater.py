...
from constants import (
    ...
    ADV_MODE, COMA_Q_GAMMA, DR_KAPPA, DR_LAMBDA,
)
...
class MAPPOUpdater:
    def __init__(
        ...
        lock_grab_ent_weight: float = 1.0,
    ):
        self.actor            = actor
        ...
        self.lock_grab_ent_weight = lock_grab_ent_weight
    ...
    def update(self, buffer, next_global_state, w_int, seeker_wr: float = 0.5,
        ...
        with torch.no_grad():
            ...
            # Short-horizon hider-team return for the COMA Q-target. Reuses
            # the tested GAE recursion at a low gamma so the effective horizon
            # is ~1/(1-COMA_Q_GAMMA) ≈ 20 steps — single-action effects are
            # then resolvable above the Q critic's prediction error.
            _, ret_ext_short = self.compute_gae(
                buffer.rewards_ext, v_ext, values_next_ext,
                buffer.dones, gamma=COMA_Q_GAMMA, gae_lambda=COMA_Q_GAMMA,
            )
            ...
            # Discounted difference-return per agent (ADV_MODE='dr'): GAE with
            # zero value baseline reduces to Sum_l (gamma*lambda)^l D_{t+l},
            # i.e. the agent's discounted exact marginal contribution.
            zero_v = torch.zeros_like(v_ext)
            dr_return, _ = self.compute_gae(
                buffer.rewards_dr, zero_v, zero_v,
                buffer.dones, gamma=0.998, gae_lambda=DR_LAMBDA,
            )
        ...
        flat_ret_short = ret_ext_short.view(-1, A)
        flat_ret_int  = ret_int.view(-1, A)
        flat_gae_adv  = gae_adv_ext.view(-1, A)
        flat_dr_ret   = dr_return.view(-1, A)
        ...
        for epoch in range(num_epochs):
            ...
            for start in range(0, batch_size, mini_batch_size):
                ...
                mb_ret_short = flat_ret_short[mb_idx]
                mb_ret_int  = flat_ret_int[mb_idx]
                mb_gae_adv  = flat_gae_adv[mb_idx]
                mb_dr_ret   = flat_dr_ret[mb_idx]
                ...
                # ── log-probs & entropy ───────────────────────────────────────
                # Grab/lock heads are weighted higher (LOCK_GRAB_ENT_WEIGHT) so
                # exploration is concentrated on object-interaction decisions —
                # the long-horizon box-manipulation sequence that phase-2 fort-
                # building requires — WITHOUT randomizing movement/rotation
                # (which would degrade the consolidated hiding policy).
                ...
                for a in range(A):
                    ...
                    entropy = entropy + (
                        ...
                        + self.lock_grab_ent_weight * (dg.entropy() + dl.entropy())
                    ).mean()
                ...
                with torch.no_grad():
                    if self.adv_mode == 'coma':
                        ...
                        mb_advantages = _std(mb_advantages)
                    elif self.adv_mode == 'dr':
                        # Difference-reward advantage: standardized team GAE
                        # (dense, aligned) + per-agent exact marginal credit
                        # (the difference-return). Each agent's advantage now
                        # differs by ITS contribution — the credit GAE lacks.
                        mb_advantages = (
                            _std(mb_gae_adv)
                            + DR_KAPPA * _std(mb_dr_ret)
                            + mb_w * mb_adv_int
                        )
                        mb_advantages = _std(mb_advantages)
                    else:
                ...
                # ── COMA critic loss (short-horizon Q-target) ─────────────────
                # Train Q on the SHORT-horizon hider return so single-action
                # counterfactual effects clear the critic's error floor.
                q_target = mb_ret_short[:, :2].mean(dim=-1, keepdim=True).detach()
